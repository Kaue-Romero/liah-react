...
import { ensureSearchHistory, readAuth, readStorageItem, removeStorageItem, writeStorageItem } from '../utils/storage';
...
export function App({ config }: AppProps) {
  ...
  const normalizedConfig = useMemo<LiahConfig>(
    () => ({
  ...
  const [activeTab, setActiveTab] = useState<'recommended' | 'store'>('recommended');
  ...
  const refreshProducts = useCallback(async () => {
    ...
    try {
      ...
      const [productResponse, categoryResponse, campaignResponse, carouselResponse] = await Promise.all([
        api.loadProducts({ config: { ...normalizedConfig, p: normalizedConfig.p, n: normalizedConfig.n }, estado: state, authId: auth.id }),
        api.loadCategories(state),
        storeMode ? api.loadCampaigns(normalizedConfig, auth.id) : Promise.resolve({ campanhas: [] }),
        storeMode ? api.loadCarousel(normalizedConfig) : Promise.resolve({ data: [] })
      ]);
  ...
  }, [auth.id, normalizedConfig, pushToast, selectedState, storeMode]);
  ...
  const refreshOrdersAndFaq = useCallback(async () => {
    ...
    const [ordersResponse, faqResponse] = await Promise.all([
      ...
      api.loadFaq(normalizedConfig.empresa || 1)
    ]);
    setOrders(Array.isArray(ordersResponse) ? ordersResponse : []);
    setFaq(faqResponse);
  }, [auth.id, auth.token, normalizedConfig.empresa]);
  ...
  const refreshUserInfo = useCallback(async () => {
    ...
    const info = await api.loadUserInfo(normalizedConfig, auth.id, auth.token, subtotal);
    setUserInfo(info);
  }, [auth.id, auth.token, normalizedConfig, subtotal]);
  ...
  useEffect(() => {
    if (!normalizedConfig.produto || !Object.keys(products).length) return;
    const product = products[String(normalizedConfig.produto)];
    if (product) {
      window.queueMicrotask(() => {
  ...
  }, [normalizedConfig.produto, products]);

  const addProduct = useCallback((product: Product, quantity = quantities[String(product.id)] || 1) => {
  ...
  const updateQuantity = useCallback((productId: number, next: number) => {
  ...
  }, [auth.id, auth.token, favorites, pushToast]);
  ...
  }, [auth.id, auth.token, refreshUserInfo]);
  ...
  }, [pushToast, setCartIds, setQuantities]);
  ...
  }, [auth.id, auth.token, refreshNotifications]);
  ...
  return (
    <div id="embed-body" className="liah-react-root">
      ...
      {storeMode && cartCount > 0 ? (
        <div id="div-carrinho" className="liah-react-cart-fab" style={{ bottom: `${normalizedConfig.carrinho_altura || 20}px` }}>
      ...
      {showBackTop ? (
        <button
          ...
          style={{ bottom: `${normalizedConfig.carrinho_altura || 20}px` }}
          type="button"
      ...
      {modal === 'product' && selectedProduct ? (
        <ProductDetailPanel
          ...
          onRate={(product, rating) => {
            void api.addRating(normalizedConfig, auth.id, product.id, rating).then(() => pushToast('Avaliação registrada.', 'success'));
          }}
        />
      ) : null}
